...
class GedLayer(nn.Module):
    def __init__(self, rings_andor_fw, node_label, node_label_dict, nb_edge_label):
        super(GedLayer, self).__init__()
        self.nb_labels = len(node_label_dict)
        self.nb_edge_labels = nb_edge_label
        self.device = 'cpu'
    ...
    def forward(self, input):
        ...
        if self.rings_andor_fw == 'rings_sans_fw':
            ring_g, ring_h = rings.build_rings(k1, cedl.size()), rings.build_rings(k2, cedl.size())
            C = ged_torch.construct_cost_matrix(g1, g2, cns, ces, cndl, cedl, self.node_label, self.node_label_dict,
                                                self.nb_edge_labels)
            S = ged_torch.mapping_from_cost_method(C, n, m, k1, k2, cns, ces, cndl, cedl, ring_g, ring_h,
                                                   self.rings_andor_fw)
