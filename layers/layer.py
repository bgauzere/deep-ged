...
class GedLayer(nn.Module):
    def __init__(self, nb_node_pair_label, nb_edge_pair_label, normalize=False, node_label="label"):
...
class Net(nn.Module):
    ...
    def forward(self, input):
        ...
        for k in tqdm(range(len(input))):
            ...
            self.ring_g, self.ring_h = rings.build_rings(g1, edgeInsDel.size()), rings.build_rings(g2,
                                                                                                   edgeInsDel.size())
            ...
            S = self.mapping_from_cost_sans_FW(n, m, g1, g2, node_costs, edge_costs, nodeInsDel, edgeInsDel, self.GraphList)
