    ...
    for t in range(nb_iter):
        ...
        for train_data, train_labels in trainloader:
            ...
            for k in tqdm(range(len(train_data))):
                # print(train_data[k])
                ged_pred[k] = model((Gs[train_data[k][0]], Gs[train_data[k][1]], train_data[k])).to(device)
        ...
        for valid_data, valid_labels in validationloader:
            ...
            for k in tqdm(range(len(valid_data))):
                # print(train_data[k])
                ged_pred[k] = model((Gs[valid_data[k][0]], Gs[valid_data[k][1]], valid_data[k])).to(device)
